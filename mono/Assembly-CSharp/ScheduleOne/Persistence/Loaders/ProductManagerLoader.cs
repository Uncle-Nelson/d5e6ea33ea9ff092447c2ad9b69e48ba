...
namespace ScheduleOne.Persistence.Loaders
{
	...
	{
		// Token: 0x06001498 RID: 5272 RVA: 0x0005BCF0 File Offset: 0x00059EF0
		public override void Load(string mainPath)
		{
			...
			if (Directory.Exists(path))
			{
				...
				for (int i = 0; i < files.Length; i++)
				{
					...
					if (base.TryLoadFile(files[i], out text, false))
					{
						...
						try
						{
							productData = JsonUtility.FromJson<ProductData>(text);
						}
						catch (Exception ex)
						{
						...
						if (productData != null)
						{
							...
							if (string.IsNullOrEmpty(productData.Name))
							{
								...
								{
									productData.Name = "Product " + Random.Range(0, 1000).ToString();
								}
								flag = true;
							}
			...
			string text2;
			if (base.TryLoadFile(Path.Combine(mainPath, "Products"), out text2, true))
			{
				ProductManagerData productManagerData = JsonUtility.FromJson<ProductManagerData>(text2);
