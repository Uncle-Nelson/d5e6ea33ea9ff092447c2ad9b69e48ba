...
namespace ScheduleOne.Persistence.Loaders
{
	...
	{
		// Token: 0x060014AE RID: 5294 RVA: 0x0005C85C File Offset: 0x0005AA5C
		public override void Load(string mainPath)
		{
			...
			string text;
			if (base.TryLoadFile(Path.Combine(mainPath, "Trash"), out text, true))
			{
			...
			if (Directory.Exists(path2))
			{
				...
				for (int k = 0; k < files2.Length; k++)
				{
					...
					if (base.TryLoadFile(files2[k], out text3, false))
					{
						...
						try
						{
							trashGeneratorData = JsonUtility.FromJson<TrashGeneratorData>(text3);
						}
						catch (Exception ex4)
						{
